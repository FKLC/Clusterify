...
var figue = function () {
	...
	function generateDendogram(tree, sep, balanced, withLabel, withCentroid, withDistance) {
		...
		if (tree.isLeaf()) {
			...
			len = sep + right_dendo[0].length + left_dendo[0].length ;
			if (withCentroid) 
				len = Math.max(centroidstr.length , len) ;
			if (withDistance) 
	...
	function agglomerate (labels, vectors, distance, linkage) {
		...
		for (p = 0 ; p < N-1 ; p++) {
			...
			const newCluster = new Node (-1, c1Cluster, c2Cluster , distMatrix[c1][c2] , newCentroid) ;
...
{
	const lines = figue.generateDendogram(this, sep, balanced,withLabel,withCentroid, withDistance) ;
	return lines.join ("\n") ;	
}
